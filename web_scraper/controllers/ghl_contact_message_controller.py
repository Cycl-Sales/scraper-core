...
class GhlContactMessageController(http.Controller):
    ...
    def download_recording(self, message_id, **kwargs):
        ...
        try:
            ...
            _logger.info(f"Raw recording_data type: {type(message.recording_data)}, length: {len(message.recording_data) if message.recording_data else 0}")
            ...
            try:
                ...
                _logger.info(f"Decoded recording_data length: {len(recording_data)} bytes")
            ...
            _logger.info(f"Serving recording for message {message_id}: {message.recording_filename}, size: {len(recording_data)} bytes")
